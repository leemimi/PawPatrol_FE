...
const SignUp = () => {
    ...
    const handleEmailVerification = async () => {
        try {
            const response = await fetch(
                `${import.meta.env.VITE_CORE_FRONT_BASE_URL}/api/v2/auth/email/verification-code`,
                {
                    method: 'POST',
                    headers: {
    ...
    const handleVerifyCode = async () => {
        try {
            const response = await fetch(
                `${import.meta.env.VITE_CORE_FRONT_BASE_URL}/api/v2/auth/email/verify`,
                {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch(
                `${import.meta.env.VITE_CORE_FRONT_BASE_URL}/api/v2/auth/sign-up`,
                {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
