...
const LoginScreen = () => {
    const socialLoginForKakaoUrl = `${import.meta.env.VITE_CORE_FRONT_BASE_URL}/oauth2/authorization/kakao`; // 카카오 로그인 요청 URL
    const socialLoginForGoogleUrl = `${import.meta.env.VITE_CORE_FRONT_BASE_URL}/oauth2/authorization/google`; // 구글 로그인 요청 URL
    const socialLoginForNaverUrl = `${import.meta.env.VITE_CORE_FRONT_BASE_URL}/oauth2/authorization/naver`; // 네이버 로그인 요청 URL
    ...
    const handleLogin = async (e) => {
        ...
        try {
            const login_response = await axios.post(
                `${import.meta.env.VITE_CORE_FRONT_BASE_URL}/api/v2/auth/login`,
                request
            )
